    ...
    axios.get("http://api.tvmaze.com/search/shows?q=" + query.value)
        .then((res) => {
            ...
            responseArray.forEach((item) => {
                ...
                // console.log(image);
                output.innerHTML += `<li class="card">${item.show.name} with rating ${item.show.rating.average}</li><br>
                <li>${item.show.genres} and currently ${item.show.status}</li>`;
                //image output works but if its null from the api the search output doesn't display property
                //icon.src += `${image}`;
            });            
        })
        .catch((error) => {
